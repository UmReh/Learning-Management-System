...
include '../_dbconnect.php';
    ...
    if(isset($_POST['add'])){
        ...
        if($fileerror==0){
            ...
            $note= round(microtime(true)).'.'.end($temp);
            move_uploaded_file($filepath,"../notes/".$note);
        }

        $course=$_POST['course'];
        $sem=$_POST['sem'];
        // $sql = "INSERT INTO `$table` (`subject`,`topic`,`course`,`sem`,`note`) VALUES ('$subject','$topic','$course','$sem','$note')";
        // $query=mysqli_query($conn,$sql);
        $sql2 = "INSERT INTO `$course` (`subject`,`topic`,`sem`,`note`,`staff`) VALUES ('$subject','$topic','$sem','$note', '$table')";
        $query2=mysqli_query($conn,$sql2);
        if($query2){
        ...
        <div class="table-content">
            <table>
                ...
                <tbody>
                   ...
                   while($result=mysqli_fetch_array($query5)){ if($result['note']!="0"){?>
                    <tr>
                        <td><?php echo $i;?></td>
                        ...
                        <td><form action="note.php?id=<?php echo $result['sno'];?>&cname=<?php echo $course_table;?>&note=<?php echo $result['note'];?>"method="POST"><input type="submit" name="delete"class="danger" value="DELETE"></form></td>
                    </tr>
                    <?php $i++;}; }}?>
                </tbody>
            </table>
