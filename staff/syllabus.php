...
include '../_dbconnect.php';
    ...
    if(isset($_POST['add'])){
        ...
        if($fileerror==0){
            ...
            $syllabus= round(microtime(true)).'.'.end($temp);
            move_uploaded_file($filepath,"../syllabus/".$syllabus);
        }

        $course=$_POST['course'];
        $sem=$_POST['sem'];
        // $sql = "INSERT INTO `$table` (`subject`,`topic`,`course`,`sem`,`syllabus`) VALUES ('$subject','$topic','$course','$sem','$syllabus')";
        // $query=mysqli_query($conn,$sql);
        $sql2 = "INSERT INTO `$course` (`subject`,`topic`,`sem`,`syllabus`,`staff`) VALUES ('$subject','$topic','$sem','$syllabus','$table')";
        $query2=mysqli_query($conn,$sql2);
        if($query2){
...
<link rel="stylesheet" href="css/style.css?v=<?php echo time(); ?> ">
    ...
    <title>Syllabus - Teacher</title>
</head>
<body>
    <?php include '_sidebar.html';?>
    <div class="home-content">
    <?php include '_nav.html';?>
        <div class="head">Syllabus</div>
        ...
        <div class="table-content">
            <table>
                ...
                <tbody>
                   ...
                   while($result=mysqli_fetch_array($query5)){ if($result['syllabus']!="0"){?>
                    <tr>
                        <td><?php echo $i;?></td>
                        ...
                        <td><a href="../syllabus/<?php echo $result['syllabus'];?>" target="_blank" rel="noopener noreferrer">Click Here</a></td>
                        <td><form action="syllabus.php?id=<?php echo $result['sno'];?>&cname=<?php echo $course_table;?>&syllabus=<?php echo $result['syllabus'];?>"method="POST"><input type="submit" name="delete"class="danger" value="DELETE"></form></td>
                    </tr>
                    <?php $i++;}; }}?>
                </tbody>
            </table>
