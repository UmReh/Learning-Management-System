...
session_start();
    ...
    if(isset($_POST['add'])){
        ...
        $course=$_POST['course'];
        $sem=$_POST['sem'];
        // $sql = "INSERT INTO `$table` (`subject`,`topic`,`course`,`sem`,`url`) VALUES ('$subject','$topic','$course','$sem','$url')";
        // $query=mysqli_query($conn,$sql);
        $sql2 = "INSERT INTO `$course` (`subject`,`topic`,`sem`,`url`,`staff`) VALUES ('$subject','$topic','$sem','$url','$table')";
        // echo $sql2;
        // die();
        $query2=mysqli_query($conn,$sql2);
        if($query2){
...
<link rel="stylesheet" href="css/style.css?v=<?php echo time(); ?> ">
    ...
    <title>Video Lecture - Teacher</title>
</head>
<body>
    <?php include '_sidebar.html';?>
    <div class="home-content">
    <?php include '_nav.html';?>
        <div class="head">Video Lecture</div>
                ...
                <tbody>
                   <?php 
                    ...
                    while($course = mysqli_fetch_array($courses)){
                        ...
                        while($result=mysqli_fetch_array($query5)){  if($result['url']!="0"){?>
                            <tr>
                                <td><?php echo $i;?></td>
                                ...
                                <td><form action="video.php?id=<?php echo $result['sno'];?>&cname=<?php echo $course_table;?>" method="POST"><input type="submit" name="delete"class="danger" value="DELETE"></form></td>
                            </tr>
                            <?php
                            $i++;}; }
                    }
